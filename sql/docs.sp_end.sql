...
--		l_json_details = '[]';
		...
		if result is not null and result > 0 then
			...
			update docs.doc set docs_ids = array_append(v_arv.docs_ids, result) where id = v_arv.parentId;

			update docs.doc set docs_ids = array_append(docs_ids, v_arv.parentId) where id = result;
...
select docs.gen_lausend_arv(75, 75)

/*

select docs.gen_lausend_arv($1, $2) as journal_id [ 75, 75 ]
select * from docs.arv1
